...
let modelResetTimer: ReturnType<typeof setTimeout> | null = null;

const switchToFallback = () => {
    if (activeModel === MODEL_FALLBACK) return;
...
const robustGenerate = async <T>(
    ...
    while (attempt < maxRetries) {
        try {
            ...
            const isServerError = errStr.includes('500') || error.status === 500 || errStr.includes('503') || error.status === 503;
            ...
            if (allowFallback && currentModel === MODEL_PRIMARY) {
                const msg = `🔄 Problema en modelo principal. Cambiando a fallback...`;
            ...
            if (waitMs > 10000) apiQueue.pause(waitMs);
        ...
        try {
            ...
            if (attempt > maxRetries) {
                throw new Error(isTimeout
                    ? `Timeout al descargar documento tras ${maxRetries + 1} intentos.`
                    : `Fallo al descargar documento: ${error.message}`);
            }
...
// ─── HELPER: Prepara una imagen/URL como inlineData para Gemini ───────────────
const prepareImagePart = async (img: string, onStatus?: (msg: string) => void): Promise<{ inlineData: { mimeType: string; data: string } }> => {
    let base64: string;
    const mimeType = img.toLowerCase().includes('.pdf') ? 'application/pdf' : 'image/jpeg';

    if (img.startsWith('http://') || img.startsWith('https://')) {
        // URL remota (Supabase Storage, etc.) → fetchear y convertir a base64
        if (onStatus) onStatus(`⬇️ Descargando documento...`);
        base64 = await fetchResourceAsBase64(img);
    } else {
        // Ya es base64 local → optimizar (solo si es imagen, no PDF)
        if (mimeType.startsWith('image/')) {
            const dataUri = img.startsWith('data:') ? img : `data:image/jpeg;base64,${img}`;
            base64 = await optimizeImage(dataUri);
        } else {
            base64 = img.includes(',') ? img.split(',')[1] : img;
        }
    }

    // Sanear: eliminar espacios/saltos de línea que rompen el base64
    base64 = base64.replace(/\s/g, '');
    if (!base64) throw new Error("Base64 vacío tras procesar imagen.");
...
export const analyzeDocument = async (
    ...
    const fallbackData = {
        comercio: "", fecha: new Date().toISOString().split('T')[0],
        total: 0, iva: 0, categoria: "Material", isStockable: false,
        description: "Introducir datos manualmente", items: []
    };

    return apiQueue.add(async () => {
        try {
            const inputs = Array.isArray(input) ? input : [input];
            if (inputs.length === 0 || inputs.some(i => !i)) throw new Error("Documento vacío o inválido.");

            const imageParts = await Promise.all(
                inputs.map(inputStr => prepareImagePart(inputStr, onStatus))
            );
            ...
            - Extrae 'unitPrice' (precio por unidad sin IVA) y 'discount' (descuento aplicado en %) SOLO si 'isMaterial' es true.`;
            ...
            const response = await robustGenerate(async (model) => {
                return genAI.models.generateContent({
                    model,
                    // ✅ FIX: contents como array con role
            ...
            console.error("Gemini API Error (analyzeDocument):", error);
            const isQuotaError = error.toString().includes('429') || error.status === 429;
            return {
                ...fallbackData,
                errorType: isQuotaError ? 'QUOTA' : 'GENERIC',
                description: isQuotaError ? "Límite de cuota alcanzado." : "Error al procesar"
            };
        }
    });
};
...
export const generateSmartBudget = async (
    description: string,
    ...
    return apiQueue.add(async () => {
        ...
        // ✅ FIX: Detecta URLs de Supabase y las descarga antes de enviar a Gemini
        if (images.length > 0) {
            const imageParts = await Promise.all(
                images.slice(0, 5).map(img => prepareImagePart(img, onStatus))
            );
        ...
        catch (e) { result = cleanAndParseJSON(response.text || "[]"); }
        ...
        try {
            ...
            const response = await robustGenerate(async (model) => {
                return genAI.models.generateContent({
                    ...
                    config: {
                        temperature: 0.3,
                        systemInstruction: 'Eres un extractor de precios de tarifas de materiales.',
                        responseMimeType: 'application/json',
...
export const parseMaterialsFromImage = async (input: string | string[]): Promise<PriceItem[]> => {
    return apiQueue.add(async () => {
        try {
            const inputs = Array.isArray(input) ? input : [input];
            // ✅ FIX: usa prepareImagePart que también maneja URLs remotas
            const imageParts = await Promise.all(inputs.map(img => prepareImagePart(img)));
...
export const calculateDrivingDistance = async (destination: string): Promise<number> => {
    try {
        ...
        const geoResponse = await fetch(geoUrl, { headers: { 'User-Agent': 'OniluzApp/1.0' } });
        ...
        if (routerData.code !== 'Ok' || !routerData.routes?.length) {
            console.warn("[Oniluz Map] No se pudo calcular la ruta.");
            return 0;
        }

        const km = Math.round(routerData.routes[0].distance / 1000);
